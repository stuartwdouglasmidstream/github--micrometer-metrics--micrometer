...
export PROJECT_VERSION=2.0.0-M1
export JAVA_HOME=/lib/jvm/java-1.8.0
export ENFORCE_VERSION=

set -- "$@" assemble publishToMavenLocal -DdisableTests -Prelease.version=2.0.0-M1 
...
if [ -n "" ]; then
  echo "Creating tag 2.0.0-M1 to match enforced version"
  git tag -m 2.0.0-M1 -a 2.0.0-M1 || true
fi
